 ...
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 *
 * Simple MVC client javascript framework.
 *
 * DEPENDANCIES:
 *     jQuery                 1.5+         required
 *     jquery.TExpreso.js     1.1+         required  (>> TExpreso.js 1.3+)
 *     docCookies.js                       required
 *
...
*/
 ...
 (function (window, $, docCookies) { 
	...
	function ClientMVC() {
		...
		if (!($ && window.TExpreso && $.TExpreso && docCookies)) { console.error(IAM_ERROR,"Missing any dependencies: JQuery 1.5+, TExpreso 1.1+, JQuery TExpreso 1.3+, docCookies"); return;};
		...
		var options = { 
				...
				dataAdapter: 	function(o){return o;}
		};
		...
		*/
		...
		/**
		* Principalmente establece como activo el route indicado luego de renderizarlo al DOM mediande TExpreso framework. Si el template implicado 
				...
				load: function(cx,q) {
					...
					for (var i in q) {
						...
						$.TExpreso.addFromUrl(q[i], function(cnt,tt) {
							if (cnt!=0) { log("<< Remote template loaded: "+tt); cx.count++; } else { console.warn(IAM_ERROR, '!! Template NOT found! (path: '+tt+')'); };
							cx.ready(cx.count);
						});
					}
				}	
		...
		*    Además, si hay dataAdapter, se injecta en cada prop valores helpers para facilitar el ensamblado de codigo html resultante con los templates TExpreso.
		*    Los helpers que comienzan con "_$is" son booleans, y los que comienzan con "_$a" contienen código final a incrustar (como attribs del tag subyacente).
		...
		* @param ns         string            especifica a partir de que namespase asignar (default root). Ej: "person" o "person.localidad", uso: ctx.setModelView(localidad,"person"); 
		* @param ovw        bool/undefined    overwrite full route model? or merge (default false = merge)
		*/
		function setModelView(data,ns,ovw) {
			if (!data) return this;
			var nsh = ns ? ns+'.':''; //namespace
			for (var i in data) { options.dataAdapter(data[i],nsh+i) };  // agregar html helpers
		...
		*/
		...
		//function dataAdapt(o) { return options.dataAdapter(o); };
		...
		*/
		 ...
		 *
		 *    Nota: cada route hereda estos atributos, aunque además en cada route puede indicársele los propios (ver whenRoute()).
		 *
		 */
		this.start = function( opts ) {
			...
			$('document').ready( function() {
				...
				if (window.dataAdapter) options.dataAdapter = window.dataAdapter; 
				else log('Warning! No data adapter was defined ! (should load dataAdapter.js)');
		...
		function log(t1,t2) {
			...
			if (!self.debugLogId) { if (t2) console.info(t1,t2); else console.info(t1); } else {$('#'+self.debugLogId).append(t1+' '+(t2?t2:'')+'<br>')}; 
		};
	...
	if (!window.ClientMVC) {
		window.ClientMVC = new ClientMVC();
		window.MVC = window.ClientMVC;  // sinonimo
	};	
	
}(window, jQuery, docCookies));
